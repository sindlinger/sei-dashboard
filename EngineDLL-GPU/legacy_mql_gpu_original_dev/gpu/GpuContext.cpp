...
WaveformResources::WaveformResources()
    : length(0),
      ...
      d_real(nullptr),
      d_imag(nullptr),
      d_work_forward(nullptr),
      d_work_inverse(nullptr),
      work_forward_bytes(0),
      work_inverse_bytes(0),
      stream_fft(nullptr),
      stream_post(nullptr),
      ready(false) {}
...
void WaveformResources::Reset() {
    ...
    SafeCudaFree(d_real);
    SafeCudaFree(d_imag);
    SafeCudaFree(d_work_forward);
    SafeCudaFree(d_work_inverse);
    d_input = nullptr;
    d_fft = nullptr;
    d_real = nullptr;
    d_imag = nullptr;
...
int GpuContext::ConfigureWaveform(size_t length) {
    ...
    // Create forward FFT plan with manual workspace allocation (allows larger windows)
    if(cufftCreate(&waveform_.plan) != CUFFT_SUCCESS) {
        waveform_.Reset();
        LogMessage("cufftCreate failed (forward plan)");
        return STATUS_PLAN_ERROR;
    }

    if(cufftSetAutoAllocation(waveform_.plan, 0) != CUFFT_SUCCESS) {
        waveform_.Reset();
        LogMessage("cufftSetAutoAllocation failed (forward plan)");
        return STATUS_PLAN_ERROR;
    }

    size_t forward_work_bytes = 0;
    if(cufftMakePlan1d(waveform_.plan,
                       static_cast<int>(length),
                       CUFFT_D2Z,
                       1,
                       &forward_work_bytes) != CUFFT_SUCCESS) {
        waveform_.Reset();
        LogMessage("cufftMakePlan1d failed (forward plan)");
        return STATUS_PLAN_ERROR;
    }

    waveform_.work_forward_bytes = forward_work_bytes;
    if(forward_work_bytes > 0) {
        if(cudaMalloc(&waveform_.d_work_forward, forward_work_bytes) != cudaSuccess) {
            waveform_.Reset();
            LogMessage("cudaMalloc failed for forward workspace");
            return STATUS_MEMORY_ERROR;
        }
        if(cufftSetWorkArea(waveform_.plan, waveform_.d_work_forward) != CUFFT_SUCCESS) {
            waveform_.Reset();
            LogMessage("cufftSetWorkArea failed (forward plan)");
            return STATUS_PLAN_ERROR;
        }
    }
    ...
    if(cufftSetStream(waveform_.plan, waveform_.stream_fft) != CUFFT_SUCCESS) {
        waveform_.Reset();
        LogMessage("cufftSetStream failed (forward plan)");
        return STATUS_PLAN_ERROR;
    }

    // Create inverse FFT plan (Z2D) with manual workspace allocation
    if(cufftCreate(&waveform_.plan_inverse) != CUFFT_SUCCESS) {
        waveform_.Reset();
        LogMessage("cufftCreate failed (inverse plan)");
        return STATUS_PLAN_ERROR;
    }

    if(cufftSetAutoAllocation(waveform_.plan_inverse, 0) != CUFFT_SUCCESS) {
        waveform_.Reset();
        LogMessage("cufftSetAutoAllocation failed (inverse plan)");
        return STATUS_PLAN_ERROR;
    }

    size_t inverse_work_bytes = 0;
    if(cufftMakePlan1d(waveform_.plan_inverse,
                       static_cast<int>(length),
                       CUFFT_Z2D,
                       1,
                       &inverse_work_bytes) != CUFFT_SUCCESS) {
        waveform_.Reset();
        LogMessage("cufftMakePlan1d failed (inverse plan)");
        return STATUS_PLAN_ERROR;
    }

    waveform_.work_inverse_bytes = inverse_work_bytes;
    if(inverse_work_bytes > 0) {
        if(cudaMalloc(&waveform_.d_work_inverse, inverse_work_bytes) != cudaSuccess) {
            waveform_.Reset();
            LogMessage("cudaMalloc failed for inverse workspace");
            return STATUS_MEMORY_ERROR;
        }
        if(cufftSetWorkArea(waveform_.plan_inverse, waveform_.d_work_inverse) != CUFFT_SUCCESS) {
            waveform_.Reset();
            LogMessage("cufftSetWorkArea failed (inverse plan)");
            return STATUS_PLAN_ERROR;
        }
    }
    ...
    oss << "Waveform configured length=" << length
        << " work_forward=" << waveform_.work_forward_bytes
        << "B work_inverse=" << waveform_.work_inverse_bytes << "B";
    LogMessage(oss.str());
    return STATUS_OK;
}
