...
pastas do WaveSpecGPU são apontadas para `Dev/`:
...
- `MQL5\Scripts` (quando existente) → `Dev/Scripts`.
...
2. `WatchdogFiles_dev_to_runtime.py --apply --once` — espelha `Dev/*` em `runtime/*`.
3. `SetupRuntimeLinks.ps1` — recria as junctions da instância atual apontando para `Dev/`.
4. `WatchdogLinks_runtime_m5folders.py --fix` — garante que todas as instâncias GUID tenham as mesmas junctions.
...
Alternar destino das junções
----------------------------

- Opção **11** do CLI: reponta todas as junções para `Dev/` (fluxo padrão de desenvolvimento).
- Opção **12**: reponta para `runtime/`, útil quando o tester precisa travar DLLs sem bloquear `Dev/bin`.
- Opção **13**: solicita um caminho customizado e recria as junções usando-o como base (mantendo a estrutura relativa de `Dev/`).
...
- `WatchdogLinks_runtime_m5folders.py --fix` corrigiu o problema de
